...
df = pd.read_csv('C:\\Users\\Marco\\Downloads\\IEA Global EV Data 2024 full.csv', delimiter=',')

# Título de la aplicación
st.title("Eliminación e Imputación de Valores por País")

# Selección de países desde un combo box
paises_seleccionados = st.multiselect('Seleccione países', df['region'].unique())

# Filtrar el DataFrame por los países seleccionados
df_filtrado = df[df['region'].isin(paises_seleccionados)]

# Mostrar las columnas disponibles para la eliminación de valores
columnas_disponibles = df_filtrado.columns.tolist()
columnas_a_eliminar = st.multiselect('Seleccione columnas para eliminar valores', columnas_disponibles[8:])
...
if len(columnas_a_eliminar) > 0:
    ...
    # Graficar la comparación de los datos actuales vs imputados
